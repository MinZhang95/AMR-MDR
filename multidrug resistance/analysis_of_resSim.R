...
library(dplyr)
pair <- "retail_Kentucky_FOX_TIO"
# (cutoff1 <- read.csv("abb_full_cutoff.csv") %>% filter(abb == "FOX") %>% pull(interm) %>% log2())
# (cutoff2 <- read.csv("abb_full_cutoff.csv") %>% filter(abb == "TIO") %>% pull(interm) %>% log2())
cutoff1 <- 4; cutoff2 <- 2
# n <- 594
...
binaryGiven <- as.data.frame(
  matrix(c(rep(c(0, 0), times = round(n * p_nn, 0)), 
           rep(c(0, 1), times = round(n * p_nr, 0)), 
           rep(c(1, 0), times = round(n * p_rn, 0)), 
           rep(c(1, 1), times = round(n * p_rr, 0))), 
         ncol = 2, 
         byrow = T)
)
colnames(binaryGiven) <- c("var1.concl", "var2.concl")
...
for (i in 1:100) {
  ...
  if (file.exists(fname)) {
    ...
    if (max(res.df[, "Rhat"], na.rm = T) < 1.03) {
      res.ls[[id]] <- res.df
      id <- id + 1
    }
  }
